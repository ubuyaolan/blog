 ...
 * $Id: template.php 1962 2011-05-04 12:18:30Z emloog $
 */
...
if($action == 'custom-top')
{
	...
	//遍历Sae目录下所有的jpg文件找出相对应的文件
	$sto=new SaeStorage();
	$num = 0;
 	while ( $ret = $sto->getList(S_DOMAIN, "upload/*", 100, $num ) ) {
         foreach($ret as $file) {
         
            if (strstr($file,'upload/top-')&& !strstr($file, '_mini.jpg')) {
            
        	$default_topimgs[] = $sto->getUrl(S_DOMAIN,$file);
    		}
             $num ++;
         }
 	}
...
if($action == 'del_top')
{
	...
	$sto=new SaeStorage();
	preg_match("/([^\\/]*)$/i",$top,$topimgname);
	$sto->delete(S_DOMAIN,"upload/".$topimgname[0]);
	$sto->delete(S_DOMAIN,"upload/".str_replace('.jpg', '_mini.jpg', $topimgname[0]));
...
if ($action == 'crop') {
	...
	$sto=new SaeStorage();
	//create topimg
	$topimg_path =  'upload/top-' . gmdate('Ym') .$time . '.jpg';
	...
	//create mini topimg
	$topimg_mini_path = 'upload/top-' .gmdate('Ym') .  $time . '_mini.jpg';
	$ret = imageCropAndResize($sto->getUrl(S_DOMAIN,$topimg_path), $topimg_mini_path, 0, 0, 0, 0, 230, 48, $width, $height);
	if (false === $ret) {
		emDirect("./template.php?action=custom-top&error_a=true");
	}

	preg_match("/([^\\/]*)$/i",$top_img,$topimgname);
	$sto->delete(S_DOMAIN,$topimgname[0]);

	$custom_topimgs = Option::get('custom_topimgs');
	array_push($custom_topimgs, $sto->getUrl(S_DOMAIN,$topimg_mini_path));

	Option::updateOption('topimg', $sto->getUrl(S_DOMAIN,$topimg_path));
	Option::updateOption('custom_topimgs', serialize($custom_topimgs));
	$CACHE->updateCache('options');
	emDirect("./template.php?action=custom-top&activated=true");
}
