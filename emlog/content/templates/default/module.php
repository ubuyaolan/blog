...
function widget_blogger($title){
	...
	<img src="<?php echo (IS_SAE ? "" : BLOG_URL).$user_cache[1]['photo']['src']; ?>" width="<?php echo $user_cache[1]['photo']['width']; ?>" height="<?php echo $user_cache[1]['photo']['height']; ?>" alt="blogger" />
	<?php endif;?>
	</div>
	<p><b><?php echo $name; ?></b>
...
function blog_att($blogid){
	...
	if(!empty($log_cache_atts[$blogid])){
		...
		foreach($log_cache_atts[$blogid] as $val){
			$att .= '<br /><a href="'.(IS_SAE ? "" : BLOG_URL).$val['url'].'" target="_blank">'.$val['filename'].'</a> '.$val['size'];
		}
	}
	echo $att;
}
