...
function uploadFile($fileName, $errorNum, $tmpFile, $fileSize, $type, $isIcon=false, $is_thumbnail=Option::IS_THUMBNAIL){
	...
	$domain = S_DOMAIN."/".Option::UPLOADFILE_PATH . gmdate('Ym') . '/';
	
	$fname = md5($fileName) . gmdate('YmdHis') .'.'. $extension;
	//初始化storage
	$storage = new SaeStorage();
	$savepath = $storage->upload($domain, auto_charset($fname, 'utf-8', 'gbk'), $tmpFile);
	if (!$savepath) {
	...
	if ($is_thumbnail) {
	    if ($isIcon && resizeImage($tmpFile, $thum, Option::ICON_MAX_W, Option::ICON_MAX_H)) {
	        $savepath = $storage->getUrl($domain, $thum);
	        resizeImage($tmpFile, $domain.'thum52-'. $fname, 52, 52);
