...
<table width="100%" id="adm_bakdata_list" class="item_list">
  ...
  <tbody>
	<?php
		...
		if(IS_SAE){
			$myfile=$sto->getAttr(S_DOMAIN,$value);
			$modtime = smartDate($myfile['datetime'],'Y-m-d H:i:s');
			$size =  changeFileSize($myfile['length']);
			$bakname = $value;
			$url = $sto->getUrl(S_DOMAIN,$bakname);
		}else{
			$modtime = smartDate(filemtime($value),'Y-m-d H:i:s');
			$size =  changeFileSize(filesize($value));
			$bakname = substr(strrchr($value,'/'),1);
			$url = "../content/backup/".$bakname;
		}
	?>
    <tr>
      <td><input type="checkbox" value="<?php echo $value; ?>" name="bak[]" class="ids" /></td>
      <td><a href="<?php echo $url; ?>"><?php echo $bakname; ?></a></td>
