...
<?php 
	$sto=new SaeStorage();
	preg_match("/([^\\/]*)$/i",$topimg,$topimgname);
?>
...
<div class=line></div>
<?php if(!file_exists('../' . $topimg) && !$sto->fileExists(S_DOMAIN,'upload/'.$topimgname[0])): ?>
<div class="error_msg">当前使用的顶部图片已被删除或损坏，请选择其它图片。</div>
<?php else:?>
<div id="topimg_preview"><img src="<?php echo (preg_match("/^http:\/\//",$topimg)?'':'../').$topimg; ?>" width="758" height="105" /></div>
<?php endif;?>
...
<div id="topimg_default">
	...
	preg_match("/([^\\/]*)$/i",$mini_imgpath,$topimgname);
	if (!file_exists('../' . $mini_imgpath) && !$sto->fileExists(S_DOMAIN,'upload/'.$topimgname[0])) {
		continue;
	}
	?>
	<div>
	<a href="./template.php?action=update_top&top=<?php echo $imgpath_url; ?>" title="点击使用该图片" >
	<img src="<?php echo (preg_match("/^http:\/\//",$mini_imgpath)?'':'../').$mini_imgpath; ?>" width="230px" height="48px" class="topTH" />
	</a>
	<?php if (!is_array($val)):?>
