 ...
 * $Id: data.php 2025 2011-09-09 16:31:03Z qiyuuu@gmail.com $
 */
...
if($action == ''){
	if(IS_SAE){
		$sto = new SaeStorage();
		$retval = $sto->getList(S_DOMAIN,"*.sql",100,0);
	}else{
...
if($action == 'bakstart'){
	...
	if(trim($sqldump)){
		...
		if($bakplace == 'local'){
			...
			if(IS_SAE){
				$sto = new SaeStorage();
				$sto->write(S_DOMAIN,"backup/".$filename,$dumpfile);//使用分布式存储备份
				if($sto->errno()!=0){
					emMsg('备份失败。Storage空间不足');
				}
				else
				{
...
if ($action == 'renewdata'){
	...
	if(IS_SAE){
		$sto = new SaeStorage();
		if(!$sto->fileExists(S_DOMAIN,$sqlfile))
		{
			emMsg('文件不存在', 'javascript:history.go(-1);',0);
		}
	}else{
	...
	checkSqlFileInfo($sqlfile,FALSE);
	bakindata($sqlfile,FALSE);
	$CACHE->updateCache();
...
if ($action == 'import'){
	...
	checkSqlFileInfo($sqlfile['tmp_name'],TRUE);
	bakindata($sqlfile['tmp_name'],TRUE);
	$CACHE->updateCache();
	emDirect("./data.php?active_import=true");
}
 ...
 * @param file $sqlfile
 */
function checkSqlFileInfo($sqlfile,$import) {
	$dumpinfo = array();
	if(IS_SAE && !$import){
		$sto = new SaeStorage();
		if($sto->fileExists(S_DOMAIN,$sqlfile))
		{
			$dumpinfo=explode("\n",$sto->read(S_DOMAIN,$sqlfile));
		}else{
			emMsg("导入失败! 读取文件失败");
		}
	}else{
