...
function uploadFile($fileName, $errorNum, $tmpFile, $fileSize, $type, $isIcon=false, $is_thumbnail=Option::IS_THUMBNAIL){
	...
	$savepath = $storage->upload(S_DOMAIN, "upload/".$fname, $tmpFile);
	if (!$savepath) {
		$err = $storage->errno();
		@unlink($tmpFile);
		emMsg('上传失败.'.$err);
	...
	//resizeImage
	$thum = "upload/".'thum-' . $fname;

	if ($is_thumbnail) {
	    if ($isIcon && resizeImage($tmpFile, $thum, Option::ICON_MAX_W, Option::ICON_MAX_H)) {
	        $savepath = $storage->getUrl(S_DOMAIN, $thum);
	        resizeImage($tmpFile, "upload/".'thum52-'. $fname, 52, 52);
...
function imageCropAndResize($src_image, $dst_path, $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h) {
		//使用SAE的图像处理函数处理图片
	$imgdata=file_get_contents($src_image);//获取图像数据
	$ex=getFileSuffix($dst_path);//获取扩展名
	$img = new SaeImage($imgdata);
	
	if($dst_x!=0||$dst_y!=0||$src_x!=0||$src_y!=0)
	{
	//剪切图片
	$imgattr=$img->getImageAttr();
	$img_w=floatval($imgattr[0]);
	$img_h=floatval($imgattr[1]);
	$lx=floatval($src_x)/$img_w;//计算x启始点
	$rx=(floatval($src_x)+$src_w)/$img_w;//计算x结束点
	$by=floatval($src_y)/$img_h;//计算y启始点
	$ty=(floatval($src_y)+$src_h)/$img_h;//计算y结束点
	$img->crop($lx,$rx,$by,$ty);
	}
	//改变图片大小
	...
	//使用分布式系统写入处理后的缩略图
	$sto=new SaeStorage();
	$sto->write(S_DOMAIN,$dst_path,$data);
	return true;
}
